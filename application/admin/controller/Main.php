...
use app\admin\model\AdminOfficer as Admin_Officer;
use app\admin\model\AdminNotice as Admin_Notice;
use app\admin\model\AdminEmail as Admin_Email;
use app\admin\model\AdminDatacenter as Admin_Datacenter;
...
class Main extends Adminbase
{
    ...
    public function index()
    {
        ...
        $CompanyList = Admin_Company::order('id desc')
            ->column('*');

        $OfficerList = Admin_Officer::order('id desc')
            ->column('*');

        $NoticeList = Admin_Notice::order('id desc')
            ->column('*');

        $EmailList = Admin_Email::order('id desc')
            ->column('*');

        $DatacenterList = Admin_Datacenter::order('id desc')
            ->column('*');
        $MonitorList = Admin_Monitor::order('id desc')
            ->column('*');

        /*var_dump($OfficerList);*/
        $this->assign('CompanyList', $CompanyList);
        $this->assign('OfficerList', $OfficerList);
        $this->assign('NoticeList', $NoticeList);
        $this->assign('EmailList', $EmailList);
        $this->assign('DatacenterList', $DatacenterList);
