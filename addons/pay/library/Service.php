...
class Service
{
    ...
    public static function submitOrder($amount, $orderid = null, $type = null, $title = null, $notifyurl = null, $returnurl = null, $method = null)
    {
        ...
        $type      = isset($params['type']) && in_array($params['type'], ['alipay', 'wechat','paypal']) ? $params['type'] : 'wechat';
    ...
    public static function getConfig($type = 'wechat')
    {
        ...
        if (isset($config['signtype']) && $config['signtype'] == 'cert') {
            //支付宝API证书 退款，提现等情况时用到
            $config['app_cert_public_key'] = APP_PATH . 'pay' . DS . 'certs' . DS . 'appCertPublicKey.crt';
            $config['alipay_root_cert']    = APP_PATH . 'pay' . DS . 'certs' . DS . 'alipayRootCert.crt';
            $config['ali_public_key']      = APP_PATH . 'pay' . DS . 'certs' . DS . 'alipayCertPublicKey.crt';
        }
